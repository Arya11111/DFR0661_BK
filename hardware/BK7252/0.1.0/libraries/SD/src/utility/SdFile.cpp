...
int bkModeChange(int cmd){
  switch(cmd){
      case O_RDONLY:  //O_READ
           return 0;
      case O_WRONLY:  //O_WRITE
           return 1;
      case O_APPEND:
           return 0x0008;
      case O_CREAT:
      ...
      case O_EXCL:
           return 0x0800;
      case 3:   //O_RDWR  O_ACCMODE
           return 2;
      case 0x13:
           return 0x0202;//FILE_WRITE
      default:
           return (int)cmd;

  }
}
int bkIsDir(const char *path){
...
uint32_t SdFile::curPosition(void) const {
    if((vol_->sdCard())->_type == TYPE_SDIO_FLASH){
        return (uint32_t) _bkFd.pos;
    }
    return curPosition_;
}
...
void SdFile::rewind(void) {
  if((vol_->sdCard())->_type == TYPE_SDIO_FLASH){
      //DIR *d;
      //rt_kprintf("fun=%s, %s", __func__,getFullpath(_bkFd.path).c_str());
      //d = opendir(getFullpath(_bkFd.path).c_str());
      //rewinddir(d);
      dfs_file_lseek(&_bkFd, 0);
      return ;
  }
...
uint8_t SdFile::close(void) {
  if((vol_->sdCard())->_type == TYPE_SDIO_FLASH){
      int result = dfs_file_close(&_bkFd);
      //rt_kprintf("fun=%s, %d\n", __func__, result);
      if(result >= 0){
          _bkFd.flags &= ~(0x01000000);
...
uint8_t SdFile::makeDir(SdFile* dir, const char* dirName) {
  if((vol_->sdCard())->_type == TYPE_SDIO_FLASH){
       ...
       d = fd_get(fd);
       result = dfs_file_open(d, getFullpath(dirName).c_str(), O_DIRECTORY | bkModeChange(O_CREAT));
       if (result < 0)
       {
           fd_put(d);
           fd_put(d);
 ...
 */
uint8_t SdFile::open(SdFile* dirFile, const char* fileName, int oflag) {
  if((vol_->sdCard())->_type == TYPE_SDIO_FLASH){
      struct dfs_fd d;
      int result, state;
      if(oflag == O_RDONLY){
          state = bkIsDir(getFullpath(fileName).c_str());
          if(state == 1) oflag = bkModeChange(O_DIRECTORY)|bkModeChange(O_RDONLY);
          //Serial.print("state=");Serial.println(state);
          
      }else{
          oflag = bkModeChange(oflag);
      }
      result = dfs_file_open(&d, getFullpath(fileName).c_str(), oflag);
      //rt_kprintf("fun=%s, state=%d re=%d %s\n", __func__, state, result, getFullpath(fileName).c_str());
      if(result < 0) {
          rt_set_errno(result);
          return false;
      }else{
...
uint8_t SdFile::openRoot(SdVolume* vol) {
  ...
  if((vol->sdCard())->_type == TYPE_SDIO_FLASH){
      vol_ = vol;
      type_ = FAT_FILE_TYPE_ROOT16;
      // rt_kprintf("fun=%s, %x, %x\n", __func__, O_DIRECTORY, O_RDONLY);
      int result = dfs_file_open(&_bkFd, "/", O_DIRECTORY);
      //rt_kprintf("fun=%s\n", __func__);
      if(result < 0){
           rt_set_errno(result);
           return false;
...
uint8_t SdFile::seekCur(uint32_t pos) {
  if((vol_->sdCard())->_type == TYPE_SDIO_FLASH){
      if(dfs_file_lseek(&_bkFd, _bkFd.pos + pos) < 0) return false;
      else return true;
  }
...
int16_t SdFile::read(void* buf, uint16_t nbyte) {
  if((vol_->sdCard())->_type == TYPE_SDIO_FLASH){
       int result;
       if(_bkFd.path == NULL) return -1;
       result = dfs_file_read(&_bkFd, buf, nbyte);
       if (result < 0)
       {
           rt_set_errno(result);
           return -1;
...
int SdFile::readDir(struct dirent* dir){
  //rt_kprintf("fun=%s, %s\n", __func__, _bkFd.path);
  return dfs_file_getdents(&_bkFd, dir, sizeof(struct dirent));
}
//------------------------------------------------------------------------------
/**
 * Remove a file.
...
uint8_t SdFile::remove(void) {
  ...
  if((vol_->sdCard())->_type == TYPE_SDIO_FLASH){
      int result;
      result = dfs_file_unlink(getFullpath(_bkFd.path).c_str());
      if (result < 0)
      {
          rt_set_errno(result);
          return false;
...
uint8_t SdFile::seekSet(uint32_t pos) {
  ...
  if((vol_->sdCard())->_type == TYPE_SDIO_FLASH){
      if(pos > _bkFd.size) pos = _bkFd.size;
      if(dfs_file_lseek(&_bkFd, pos) < 0) return false;
      else return true;
  }
...
uint8_t SdFile::isOpen(void) const {
  //rt_kprintf("func=%s, %d %x  %d\n", __func__, (vol_->sdCard())->_type,_bkFd.flags, type_);
  if((vol_->sdCard())->_type == TYPE_SDIO_FLASH){
      return (_bkFd.flags & 0x01000000) ? true : false;
  }

...
class SdFile : public Print {
 public:
  ...
  SdFile(void) : type_(FAT_FILE_TYPE_CLOSED) {
    memset(&_bkFd, 0, sizeof(struct dfs_fd));
  }
  /**
  ...
  uint8_t open(SdFile* dirFile, const char* fileName, int oflag);
...
class SdVolume {
 public:
  ...
  if(dev->_type == TYPE_SDIO_FLASH) return init(dev, 0);
...
//
  ...
  uint8_t isEOC(uint32_t cluster) const {
    if(sdCard_->_type == TYPE_SDIO_FLASH) return 0;

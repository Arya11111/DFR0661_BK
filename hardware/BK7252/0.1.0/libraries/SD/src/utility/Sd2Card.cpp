...
uint8_t Sd2Card::init(uint8_t sckRateID, uint8_t chipSelectPin, uint8_t storageType, uint32_t freq) {
  ...
  if(_type == TYPE_SDIO_FLASH){
     ...
     if(sd0 != NULL){
         //rt_kprintf("sd is not NULL  %s\n", dfs_filesystem_get_mounted_path(sd0));
         if(String(dfs_filesystem_get_mounted_path(sd0)) == "/") return true;

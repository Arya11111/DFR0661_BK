...
private:
        ...
        size_t fillBuffer()
        {
            if(!_buffer){
                ...
                if(!_buffer) {
                    WIFI_WLAN_DEBUG("Not enough memory to allocate buffer");
                    _failed = true;
                    return 0;
                }
            }
...
size_t WiFiClient::write(Stream &stream)
{
    ...
    while(available){
        ...
        toWrite = stream.readBytes((char *)buf, toRead);
...
void WiFiClient::flush() {
    ...
    while(a){
        ...
        res = lwip_recv(fd(), buf, toRead, MSG_DONTWAIT);
        if(res < 0) {
            WIFI_WLAN_DEBUG("fail on fd %d, errno: %d, \"%s\"", fd(), errno, strerror(errno));
            stop();
            break;
...
uint8_t WiFiClient::connected()
{
    if (_connected) {
        uint8_t dummy;
        int res = lwip_recv(fd(), &dummy, 0, MSG_DONTWAIT);
...
IPAddress WiFiClient::remoteIP(int fd) const
{
    ...
    lwip_getpeername(fd, (struct sockaddr*)&addr, &len);
...
IPAddress WiFiClient::localIP(int fd) const
{
    struct sockaddr_storage addr;
    socklen_t len = sizeof addr;
    lwip_getsockname(fd, (struct sockaddr*)&addr, &len);
    struct sockaddr_in *s = (struct sockaddr_in *)&addr;
    return IPAddress((uint32_t)(s->sin_addr.s_addr));
}

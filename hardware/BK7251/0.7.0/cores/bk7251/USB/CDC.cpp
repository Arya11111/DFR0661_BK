...
{
  epType[0] = USB_ENDPOINT_TYPE_INTERRUPT | USB_ENDPOINT_IN(0);//中断输入控制
  epType[1] = USB_ENDPOINT_TYPE_BULK | USB_ENDPOINT_OUT(0);//批量输出
  epType[2] = USB_ENDPOINT_TYPE_BULK | USB_ENDPOINT_IN(0);//批量输入
...
int Serial_::read(void)
{
    ...
    return 1;//usb.recv(CDC_ENDPOINT_OUT);
}
...
size_t Serial_::readBytes(char *buffer, size_t length)
{
    ...
    while (count < length)
    {
        uint32_t n = 0;//usb.recv(CDC_ENDPOINT_OUT, buffer+count, length-count);
...
{
    uint32_t r = 1;//usb.send(CDC_ENDPOINT_IN, buffer, size);
